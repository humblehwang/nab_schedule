...
class Scheduler:
    ...
    def __init__(self, csvFile='data/teams-new.csv', testSchedule=False):
        """
    ...
    def assignSchedule(self ,teams, conferences, divisions, distances,trace,nbaSchedule,nbaSchedule_name,dateSchedule,homeTeam):
        self.teams, self.conferences, self.divisions  = teams, conferences, divisions
        self.distances = distances
        self.trace = trace
        self.nbaSchedule = nbaSchedule
        self.nbaSchedule_name = nbaSchedule_name
        self.dateSchedule = dateSchedule
        self.homeTeam = homeTeam
        
    def showSchedule(self):
        for index in range(len(self.nbaSchedule)):
            print(self.nbaSchedule[index],self.dateSchedule[index])
    ...
    def totalTriples(self):
        ...
        # Calculate total triples for all teams
    def totalFourInFive(self):
        total = 0
        for team in self.teams.values():
            total += team.fourInFive()
        return total
    ...
    def getRandomDate(self, team1, team2):
        ...
        nextDate = newDate+timedelta(days=1)


        okLast = lastDate not in team1.teamCalendar.keys()
        while okLast:
            if lastDate == date(2016,10,24):
                lastDate = date(2016,10,25)
                break
        ...
        okNext = nextDate not in team1.teamCalendar.keys()
        while okNext:
            if nextDate == date(2017,4,13):
                nextDate = date(2017,4,12)
                break
        ...
        if not self.teams[team1.name].teamCalendar[newDate] and not self.teams[team2.name].teamCalendar[newDate]:
            if not self.teams[team1.name].teamCalendar[nextDate] and not self.teams[team1.name].teamCalendar[lastDate]:
                found = True
    ...
    def costFn(self, a=0, b=100, c=1000000, d=1000):
        totalDistance = self.totalDistanceAll()
        totalBTB = util.totalBackToBacks(self.teams)
        totalTriples = self.totalTriples()
        total4In5 = self.totalFourInFive()
        btbSTD, distSTD = self.getStandardDevs()
        #cost = a * totalDistance + b * totalBTB + c * totalTriples + d * (btbSTD * 4000 + distSTD)
        #print(totalDistance ,totalBTB , totalTriples , btbSTD ,distSTD)
        cost = a * totalDistance + b * totalBTB + c * totalTriples + d * total4In5
        #print(cost)
        #print(cost,totalDistance ,totalBTB , totalTriples ,total4In5)
    ...
    def fourInFivecheck(self, team, date):
        global index
        #print("Hi Im here", index)
        index = index + 1

        previous1 = date - timedelta(days=1)
        previous2 = previous1 - timedelta(days=1)
        previous3 = previous2 - timedelta(days=1)
        previous4 = previous3 - timedelta(days=1)
        previous = [previous1,previous2,previous3,previous4]

        nextday1 = date + timedelta(days=1)
        nextday2 = nextday1 + timedelta(days=1)
        nextday3 = nextday2 + timedelta(days=1)
        nextday4 = nextday3 + timedelta(days=1)        
        nextday = [nextday1,nextday2,nextday3,nextday4]
        
        check1 = [previous1,previous2,previous3,previous4]
        check2 = [previous2,previous3,previous4,nextday1]
        check3 = [previous3,previous4,nextday1,nextday2]
        check4 = [previous4,nextday1,nextday2,nextday3]
        check5 = [nextday1,nextday2,nextday3,nextday4]
        ...
        for c in check:
            count = 0
            for day in c:
                if day not in team.teamCalendar:
                    continue
                else:
                    if team.teamCalendar[day] == True:
                        count = count + 1
            if count >= 3:
                return False    
        ...
        # checks if a date would result in a triple for a team
    """     
    def sixInEightcheck(self, team, date):
        global index
        #print("Hi Im here", index)
        index = index + 1
        ...
        previous3 = previous2 - timedelta(days=1)
        previous4 = previous3 - timedelta(days=1)
        previous5 = previous4 - timedelta(days=1)
        previous6 = previous5 - timedelta(days=1)
        previous7 = previous6 - timedelta(days=1)
        previous = [previous1,previous2,previous3,previous4,previous5,previous6,previous7]

        nextday1 = date + timedelta(days=1)
        nextday2 = nextday1 + timedelta(days=1)
        nextday3 = nextday2 + timedelta(days=1)
        nextday4 = nextday3 + timedelta(days=1)        
        nextday5 = nextday4 + timedelta(days=1)
        nextday6 = nextday5 + timedelta(days=1)
        nextday7 = nextday6 + timedelta(days=1)
        nextday = [nextday1,nextday2,nextday3,nextday4,nextday5,nextday6,nextday7]
        
        check1 = [previous1,previous2,previous3,previous4,previous5,previous6,previous7]
        check2 = [previous2,previous3,previous4,previous5,previous6,previous7,nextday1]
        check3 = [previous3,previous4,previous5,previous6,previous7,nextday1,nextday2]
        check4 = [previous4,previous5,previous6,previous7,nextday1,nextday2,nextday3]
        check5 = [previous5,previous6,previous7,nextday1,nextday2,nextday3,nextday4]
        check6 = [previous6,previous7,nextday1,nextday2,nextday3,nextday4,nextday5]    
        check7 = [previous7,nextday1,nextday2,nextday3,nextday4,nextday5,nextday6]    
        check8 = [nextday1,nextday2,nextday3,nextday4,nextday5,nextday6,nextday7]    
    ...
    def randomStart(self):
        ...
        for team in self.teams.values():
            ...
            for divOpp in self.divisions[team.division]:
                if team.name != divOpp.name:
                    ...
                    i = 0
                    while i < 2:
                        self.nbaSchedule.append([divOpp,team])
                        self.nbaSchedule_name.append([divOpp.name,team.name])
                        self.homeTeam.append(team)
                        i += 1
            ...
            for nonConfOpp in self.conferences[otherConf]:
                ...
                i = 0
                while i < 1:
                    self.nbaSchedule.append([nonConfOpp,team])
                    self.nbaSchedule_name.append([nonConfOpp.name,team.name])
                    self.homeTeam.append(team)
                    # increment i
                    i += 1
            ...
            for commonNonDivOpp in team.commonNonDivOpps:
                i = 0
                while i < 2:
                    self.nbaSchedule.append([commonNonDivOpp,team])
                    self.nbaSchedule_name.append([commonNonDivOpp.name,team.name])
                    self.homeTeam.append(team)
                    i += 1
            #print("Before trip:",self.totalTriples())
            ...
            for rareNonDivOpp in team.rareNonDivOpps:
                i = 0
                while i < 1:
                    self.nbaSchedule.append([rareNonDivOpp,team])
                    self.nbaSchedule_name.append([rareNonDivOpp.name,team.name])
                    self.homeTeam.append(team)
                    i += 1
            #print("After trip:",self.totalTriples())
        ...
        for team in self.teams.values():
            for h in team.HA[0]:
                i = 0
                while i < 1:
                    self.nbaSchedule.append([h,team])
                    self.nbaSchedule_name.append([h.name,team.name])
                    self.homeTeam.append(team)
                    # increment i
                    i += 1
    ...
    def setCommonNonDivOpps(self, team):
        for t in self.teams.values():
            if t.name in team.common:
                team.commonNonDivOpps.append(t)
    ...
    def setRareNonDivOpps(self, team):
        for t in self.teams.values():
            ...
            if t.conference == team.conference and t.division != team.division and t.name not in team.common:
                team.rareNonDivOpps.append(t)
        #print("Rare:",team,":",team.name,team.rareNonDivOpps)
    ...
    def initializeHA(self, team):
        for t in self.teams.values():
            if t.name in team.ha0:
                team.HA[0].append(t)
            
            if t.name in team.ha1:
                team.HA[1].append(t)
...
class Team:
    ...
    """
    def __init__(self, name, conference, division, location,common,ha0,ha1):
    ...
    """
        # Iterate over self.schedule and calculate number of back to backToBacks
    def fiveInEight(self):
        fiveinEights = 0
        self.schedule = util.sortSchedule(self.schedule)
        for i in xrange(len(self.schedule) - 4):
            g1 = self.schedule[i].date
            g2 = self.schedule[i+4].date
            timeDelta = g2 - g1
            if timeDelta.days == 8:
                fiveinEights += 1
        return fiveinEights
    ...
    def fourInFive(self):
        count = 0
        self.schedule = util.sortSchedule(self.schedule)
        for i in xrange(len(self.schedule) - 3):
            g1 = self.schedule[i].date
            g2 = self.schedule[i+3].date
            timeDelta = g2 - g1
            if timeDelta.days == 4:
                count += 1
        return count    
